#ifndef __LINE_LOCAL_PARAMETERIZATION_H
#define __LINE_LOCAL_PARAMETERIZATION_H
...
bool LineLocalParameterization::Plus(const double *x, const double *delta, double *x_plus_delta) const
{
    ...
    Eigen::Matrix3d Rz;
    Rz << 
        cos(_delta_theta(2)), -sin(_delta_theta(2)), 0,
        sin(_delta_theta(2)), cos(_delta_theta(2)), 0,
        0, 0, 1;

    Eigen::Matrix3d Ry;
    Ry << 
        cos(_delta_theta(1)), 0, sin(_delta_theta(1)),
        0, 1, 0,
    ...
    delta_W << 
        cos(_delta_phi), -sin(_delta_phi),
        sin(_delta_phi), cos(_delta_phi);
