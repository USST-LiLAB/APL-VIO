...
{
    const Eigen::Map<const Eigen::Vector3d> tic(&parameters[0][0]);
    const Eigen::Map<const Eigen::Quaterniond> qic(&parameters[0][3]); // array order: (x, y, z, w)

    const Eigen::Map<const Eigen::Vector3d> tic2(&parameters[1][0]);
    const Eigen::Map<const Eigen::Quaterniond> qic2(&parameters[1][3]); // array order: (x, y, z, w)

    const double inv_dep_i = parameters[2][0];

    const double td = parameters[3][0];


    /* project feature from left image of frame_i to right image of frame_i */
...
#else
    double inv_dep_j = 1.0 / pts_camera_j.z();
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jaco_ex.rightCols<3>() = ric2.transpose() * ric * -math_utils::skewSymmetric(pts_camera_i);
